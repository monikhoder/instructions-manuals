<x-app-layout>
        ...
        <div class="bg-white col-span-12 lg:col-span-6 shadow-md sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="id" name="id" value="{{$category->id }}" readonly class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700">Category Name</label>
                    <input type="text" id="name" name="name" value="{{$category->name }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @if ($errors->has('name'))
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{$category->description }}</textarea>
                </div>
            </div>
        </div>
                            ...
                            @else
                                @if ($category->image)
                                    <div class="flex flex-row w-full gap-2 items-center mt-1" id="image-preview">
                                        <img src="{{ asset($category->image) }}" alt="Category Image" class="w-6 h-6 object-cover rounded">
                                        <span class="text-gray-700 ms-2">{{ basename($category->image) }}</span>
    ...
    <div class="flex justify-end gap-4 mt-5">
        <a href="{{route('admin.category')}}" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 ">Back</a>
