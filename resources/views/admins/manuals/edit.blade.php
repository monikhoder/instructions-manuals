<x-app-layout>
   <form action="{{route('admin.manual.update', ["manual"=> $manual])}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white col-span-12 lg:col-span-6 shadow-md sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="title" name="title" value="{{old('title') ?? $manual->title}}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @if ($errors->has('title'))
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{old('description') ?? $manual->description}}</textarea>
                </div>
                <div class="mb-4">
                    ...
                    <select id="brand_id" name="brand_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach($brands as $brand)
                            <option value="{{$brand->id}}" {{$manual->brand_id == $brand->id ? 'selected' : '' }}>{{$brand->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select id="category_id" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" {{ $manual->category_id == $category->id ? 'selected' : '' }}>{{$category->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select id="language" name="language" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="en" {{ $manual->language == 'en' ? 'selected' : '' }}>English</option>
                        <option value="km" {{ $manual->language == 'km' ? 'selected' : '' }}>Khmer</option>
                    </select>
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500">
                            ...
                            @else
                                @if ($manual->image)
                                    <div class="flex flex-row w-full gap-2 items-center mt-1" id="image-preview">
                                        <img src="{{ asset($manual->image) }}" alt="Manual Image" class="w-6 h-6 object-cover rounded">
                                        <span class="text-gray-700 ms-2">{{ basename($manual->image) }}</span>
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label  class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold">Manual File</span> can not make update</p>
                                <p class="text-sm">{{basename($manual->file_path)}}</p>
    ...
    <div class="flex justify-end gap-4 mt-5">
        ...
        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Update Manual</button>
    </div>
    </form>
</x-app-layout>
